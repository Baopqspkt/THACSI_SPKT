...
function points = myHarris(pic)
    ...
    % output:the row and col NÃ—2 matrix of the feature point
...
function drawHarrisCorner(pic1, points1, pic2, points2)
    % function:draw Harris feature pointsâ€™ match connection
    % input:
    % pic1ã€? pic2:the images need stitching
    % points1ã€? points2: Harris feature points position
    X1 = points1(:, 2);
    Y1 = points1(:, 1);
    X2 = points2(:, 2);
    Y2 = points2(:, 1);
...
function des = myHarrisCornerDescription(pic, points)
    ...
    % Input:
    % pic:source image
    % points:feature pointsâ€™ position
    % Output:
    % des: 8Ã—N matrix describing Harris feature points
    if length(size(pic)) == 3
        pic = rgb2gray(pic);
    end
...
function matchs = myMatch(des1, des2)
    ...
    % input:
    % des1ã€? des2:feature point description matrix
    % Output:
...
function drawLinedCorner(pic1, loc1, pic2, loc2)
    ...
    % Input:
    % pic1ã€? pic2:image to need stitching
    % loc1ã€? loc2:position of the paired points
    X1 = loc1(:, 2);
    Y1 = loc1(:, 1);
    X2 = loc2(:, 2);
    Y2 = loc2(:, 1);
...
function [newLoc1, newLoc2] = pointsSelect(loc1, loc2)
    ...
    % Input:
    % loc1ã€? loc2:position of coarse match points
    % Output:
    % newLoc1ã€?newLoc2:position of fine match points
...
function im = picMatched(pic1, newLoc1, pic2, newLoc2)
    ...
    % Input:
    % pic1ã€? pic2: images need stitching
    % newLoc1ã€?newLoc2:new position of feature points
    % Output:
